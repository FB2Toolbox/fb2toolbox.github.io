...
namespace FB2ToolboxUnitTest
{
    ...
    public class FB2FileAdvancedTests
    {
        ...
        {
            // Arrange
            FB2File file1 = new FB2File(@"TestFiles\?????????? ?????? - ????????.fb2");
            FB2File file2 = new FB2File(@"TestFiles\?????????? ?????? - ????????.fb2");
        ...
        {
            // Arrange
            FB2File file = new FB2File(@"TestFiles\?????????? ?????? - ????????.fb2");
        ...
        {
            // Arrange
            FB2File file = new FB2File(@"TestFiles\?????????? ?????? - ????????.fb2.zip");
        ...
        public void FB2File_FileInformation_ProvidesCorrectDetails()
        {
            ...
            Assert.AreEqual("?????????? ?????? - ????????.fb2", fileInfo.Name, "File name should match");
        ...
        public void FB2File_UpdateFileInfo_ChangesFileInformation()
        {
            ...
            // Act
            file.UpdateFileInfo(@"TestFiles\?????????? ?????? - ????????.fb2.zip");
            ...
            Assert.AreEqual("?????????? ?????? - ????????.fb2.zip", newName, "New file name should match");
        }
